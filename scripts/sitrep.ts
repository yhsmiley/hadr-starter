// A1 entrypoint (SLICES.md V1), and the deterministic half of the
...
// only if it did." This script never calls a model. It ingests, fuses,
// writes state, renders dashboard.html from whatever narratives already
// exist (satisfying A6.2 -- the freshness/health line updates today
// regardless), and signals whether the `sitrep` skill needs to run.
...
async function logRawPayloads(rawResponses: unknown[], nowUtc: string): Promise<void> {
  await fs.mkdir(RAW_LOG_DIR, { recursive: true });
  const datePart = nowUtc.slice(0, 10);
  await fs.writeFile(
    path.join(RAW_LOG_DIR, `usgs-${datePart}.json`),
...
async function main(): Promise<void> {
  ...
  const { events, rawResponses } = await fetchUsgsEvents();
  await logRawPayloads(rawResponses, nowUtc);

  const incidents = fuse(events, priorIncidents, nowUtc);
